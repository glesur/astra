...
class VTKDataset(object):
    ...
    def _load(self, fh):
        if self._dataset_type in ("RECTILINEAR_GRID", "STRUCTURED_GRID", "STRUCTURED_POINTS"):
            self._load_hydro(fh)
    ...
    def _load_hydro(self, fh):
        ...
        if self._dataset_type == "STRUCTURED_POINTS":
            # In this case, the coordinates are implicit and we need to reconstruct them.
            s = fh.readline()
            slist = s.decode("utf-8").split() # ORIGIN X0 Y0 Z0
            self._xbeg = float(slist[1])
            self._ybeg = float(slist[2])
            self._zbeg = float(slist[3])
            s = fh.readline()
            slist = s.decode("utf-8").split() # SPACING DX DY DZ
            self._dx = float(slist[1])
            self._dy = float(slist[2])
            self._dz = float(slist[3])

            self.x = np.arange(self.nx)*self._dx + self._xbeg
            self.y = np.arange(self.ny)*self._dy + self._ybeg
            self.z = np.arange(self.nz)*self._dz + self._zbeg
            self.xl = self.x
            self.yl = self.y
            self.zl = self.z
